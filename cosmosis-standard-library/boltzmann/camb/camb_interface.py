...
def extract_initial_power_params(block, config, more_config):
    ...
    # print("QWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW init_power ", init_power)
    return init_power
...
def extract_camb_params(block, config, more_config):
    ...
    if want_perturbations:
        ...
        # MODDDD, SET z_max = 2.0, z_mid = 1.0
        # z = np.concatenate((np.linspace(0, 
        #                             1, 
        #                             100, 
        #                             endpoint=False),
        #                 np.linspace(1.0, 
        ...
        # p.set_matter_power(redshifts=z, nonlinear=config["NonLinear"] in ["NonLinear_both", "NonLinear_pk"], kmax=20.0) #MODDDDD
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW **more_config[tansfer_params] ", **more_config["tansfer_params"])
        # **more_config["transfer_params"] : kmax = VALUE OF KMAX
        # nonlinear=config["NonLinear"] in ["NonLinear_both", "NonLinear_pk"] OUTPUTS False
        # THIS IS RUN!!
...
def save_derived_parameters(r, block):
    ...
    for cosmosis_name, CAMB_name, scaling in [("h0"               , "h",               1),
        ...
        if block.has_value(names.cosmological_parameters, cosmosis_name):
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ cosmosis_name : ",cosmosis_name)#MODDDDD
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ CAMB_value : ",CAMB_value)#MODDDDD
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ mnu : ", block["cosmological_parameters","mnu"])#MODDDDD
        ...
        # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ cosmosis_name ", cosmosis_name)
        # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ CAMB_value ", CAMB_value)
        block[names.cosmological_parameters, cosmosis_name] = CAMB_value
    # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW omega_c = ", block[names.cosmological_parameters, "omega_c"])    
    # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW omega_b = ", block[names.cosmological_parameters, "omega_b"])
    # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW omega_m = ", block[names.cosmological_parameters, "omega_m"])
    # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW omega_t = ", block[names.cosmological_parameters, "omega_c"]+block[names.cosmological_parameters, "omega_b"])
    # print("??????????? : ", block[names.cosmological_parameters, "omega_m"] == block[names.cosmological_parameters, "omega_c"]+block[names.cosmological_parameters, "omega_b"])
...
def save_distances(r, block, more_config):
    ...
    # np.savetxt("test.txt",z_background) #MODDDDDD
...
def compute_growth_factor(r, block, P_tot, k, z, more_config):
    ...
    # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ kmin = ",kmin) # kmin = 7.25e-05
    # np.savetxt("dpl_kids.txt",D) #MODDDD
    return D
...
def save_matter_power(r, block, more_config):
    ...
    for transfer_type in more_config['power_spectra']:
        ...
        # k = np.logspace(np.log10(kcalc[0]), np.log10(20.0), more_config['nk']) #MODDDD
        ...
        if p.NonLinear is not camb.model.NonLinear_none:
            ...
            block.put_grid(section_name, "z", z, "k_h", k, "p_k", p_k)
            # MODDDDDDDDDDDD BELOW
            if k[-1]==0.15 :
                block[section_name, "k015"] = True
            else :
                block[section_name, "k015"] = False
...
def execute(block, config):
    block["distances", "me"] = False #MODDDDD, I think I put this in because I need "me" in the section "distances", but in distance_mod2 this is not there
                                     # Regardless, this is going to be updated again in the distance_mod that comes later, so no worries.
