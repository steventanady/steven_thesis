...
def setup(options):
    z_cut = float(options.get(option_section, "z_cut")) # MODDD
...
def execute(block, config):
    ...
    for name, data in list(config.items()):
        if name == "z_cut" : # MODDDD
            continue
        z, nz = data
        ##########################
        # MOD, SELECTING SOURCE ONLY UP TO A CERTAIN REDSHIFT
        # print(nz.shape)
        mask = z<zcut
        z = z[mask]
        nz = nz[:,mask]
        ...
        block[name, "nz"] = ns
        block[name, "z"] = z
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW", name)
