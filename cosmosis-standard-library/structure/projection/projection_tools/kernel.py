...
class KernelSpline(object):
    def __init__(self, x, y, clip=1.e-6, ymin=1.e-12, 
        norm=True, is_pos=True, xmin_clipped_min=5.0): #xmin_clipped_min=5.0 originally
        """
        ...
        # print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX cumsum_y.shape", self.x[pos_diff].shape)
...
class TomoNzKernel(object):
    ...
    def from_block(cls, block, section_name, norm=True):
        ...
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW z : ",z) # z goes from 0 to 5.925
        nzs = []
    ...
    def set_nofchi_splines(self, chi_of_z, dchidz, clip=1.e-6):
        chi = chi_of_z(self.z)
        # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", chi_of_z(self.z))
        # print("WWWWWWWWWWWWWWW chi : ",chi_of_z(6.0))
        dchidz = dchidz(self.z)
        self.nchi_splines = {}
        for i in range(1, self.nbin+1):
            self.nchi_splines[i] = KernelSpline(chi, self.nzs[i]/dchidz, clip=clip) 
            # print("WWWWWWWWWWWWWWWWWWWWWW self.nchi_splines[i].xmax : ", self.nchi_splines[i].xmax)
            # print("WWWWWWWWWWWWWWWWWWW self.nchi_splines[i] : ", self.nchi_splines[i])
    ...
    def set_combined_shear_ia_splines(self, chi_of_z, dchidz, a_of_chi, 
        ...
        if len(self.nchi_splines) == 0:
            self.set_nofchi_splines(chi_of_z, dchidz, clip=clip)
            # print("AAAAAAAAAAAAAAAAAAAAAAAAAAAA THIS IS RUN!!!!") 
        self.shear_ia_splines = {}
        for i in range(1, self.nbin+1):
            nchi_spline = self.nchi_splines[i]
            nchi = int(np.ceil(nchi_spline.xmax/dchi))
            # print(f"DEBUGGGGGGGGGGGGGGGGGGGGGGGGGGGG nchi_spline.xmax = {nchi_spline.xmax}")
            chi_vals = np.linspace(0., nchi_spline.xmax, nchi)
            a_vals = a_of_chi(chi_vals)
            w_of_chi_vals = self.get_wofchi_vals(chi_vals, a_vals, nchi_spline)
            # np.savetxt("avals_me.txt", a_vals) #MODDDD
            ...
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZ, F_of_chi_spline(chi_vals) = ",F_of_chi_spline(chi_vals))
            w_of_chi_vals += ia_kernel_vals
            #For a negative IA amplitude, the kernel may 
            #have negative values, so call KernelSpline with
            #is_pos=False.
            #MODDDDDDDDDDDDDDDD
            # print(f"DEBUGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG len(chi_vals) = {chi_vals.shape}, len(w_of_chi_vals) = {w_of_chi_vals.shape}")
            
            #################
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZ, chi_vals = ",chi_vals)
