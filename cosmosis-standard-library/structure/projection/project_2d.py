...
class Power3D(object):
    ...
    def __init__(self, suffix=""):
        ...
        """
        self.section_name = self.get_section_name(suffix) # IMPORTANTLY MODIFIED, NO SECOND PARAMETER
    ...
    @classmethod
    def get_section_name(cls, suffix="", lin=True): # IMPORTANTLY MODIFIED : lin=False -> lin=True
        if lin is True :
            print("USING LINEAR MATTER POWER SPECTRUM") 
        else :
    ...
    def load_from_block(self, block, chi_of_z):
        ...
        # MODDDD
        # Positive-definite expansion function, but numericall error in interpolation results in not positive-definite comoving distance
        # For entries that has lower value than the one before, set its value equal to the one before plus 1
        if np.any(np.diff(chi_of_z(z))<=0) :
            entries = np.where(np.diff(chi_of_z(z))<=0)[0]
            chi_of_z = chi_of_z(z)
            for i in entries :
                chi_of_z[i] = chi_of_z[i-1] + 0.01*(chi_of_z[i+1]-chi_of_z[i-1])
            self.chi_vals = chi_of_z
        else :
            self.chi_vals = chi_of_z(z)
        # np.savetxt("chiz_lcdm.txt",self.chi_vals)
        # ORIGINAL BELOW
        # self.chi_vals = chi_of_z(z)
        ...
        self.pk_vals = pk
        # np.savetxt("pkz66.txt",self.pk_vals)
    ...
    def set_chi_logk_spline(self):
        ...
        """
        # np.savetxt("test.txt",self.pk_vals)
        self.chi_logk_spline = interp.RectBivariateSpline(self.chi_vals, self.logk_vals, self.pk_vals)
        # np.savetxt("pk_z6_many.txt",self.pk_vals)
        # np.savetxt("chiz2.txt",self.chi_vals)
        # zzz = np.linspace(0,4000,200)
        # test = self.chi_logk_spline(zzz,self.logk_vals)
        # print(self.logk_vals)
        # np.savetxt("testz6.txt",test)
...
def get_lensing_prefactor(block):
    ...
    shear_scaling = 1.5 * (100.0*100.0)/(c_kms*c_kms) * omega_m # No h0?
...
class Spectrum(object):
    ...
    def nbins(self):
        ...
        # MODIFICATION
        ######
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW", type(self.source))
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWW", self.source, "na : ",na, "nb ",nb)
        # print("sample_a : ", self.sample_a)
        # print("sample_b : ", self.sample_b)
        # print("power key : ",self.power_key)
        ######
        return na, nb
    ...
    def get_prefactor(self, block, bin1, bin2):
        ...
        prefactor = 1
        # first prefactor
        # print("ZZZZZZZZZZZZZZZZZZZ ",self.prefactor_type[0], self.prefactor_type[1])
    ...
    def compute_limber(self, block, ell, bin1, bin2, dchi=None, sig_over_dchi=100.,
        ...
        if chimax is None:
            ...
            # print(f"chimin : {chimin}, chimax : {chimax}")
        if dchi is None:
        ...
        # np.savetxt("cell_cheb.txt",c_ell)
        return c_ell
    ...
    def compute_exact(self, block, ell, bin1, bin2, dlogchi=None,
        ...
        print("COMPUTE EXACT IS ACTUALLY NOT DONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1")
        """
...
class SpectrumCalculator(object):
    ...
    def parse_requested_spectra(self, options):
        ...
        for spectrum in self.spectrumType:
            ...
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", option_name)
            # print("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", spectrum)
            try:
                value = options[option_section, option_name]
                #MODIFICATION
                ###
                # print("WWWWWWWWWWWWWWWWWWWWWvalue : ", option_name)
                ###
            ...
            for value in values:
                try:
                    ...
                    for power_suffix, only_bins in zip(suffices, only_bins_list):
                        ...
                        #MODIFICATION
                        ###
                        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW s : ",s)
                        ###
                        ...
                        if option_name in self.auto_only_option_names:
                            ...
                            print("THIS IS NOT RUN!!!!!!!!!!!!!!!!!!!!!!!!!! s.section_name", s.section_name)
    ...
    def load_distance_splines(self, block):
        ...
        # np.savetxt("11.txt",chi_distance)
        chi_distance *= h0
        ...
        #MODDDDDDDDDDDD
        me = block["distances","me"]
        if (me == True) :
            a_distance = a_distance[::-1]
            # print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAASSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS")
        ################
        # np.savetxt("report.txt",chi_distance) #MODDDDD
        ...
        # np.savetxt("z_distance.txt",z_distance)
        # np.savetxt("chi_distance6.txt",chi_distance)
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWW chi(0)] : ",self.chi_of_z(0))
        # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWW z_distance[0] : ",z_distance[0])
    ...
    def load_kernels(self, block):
        ...
        for key in self.req_kernel_keys:
            ...
            if kernel_type == "N":
                ...
                #MODIFICATION
                ###
                # print("RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR IGNORED!")
                ###
                ...
                # MODDDDDDDDDDDDDDDDD
                # Positive-definite expansion function, but numerical error in interpolation results in not positive-definite comoving distance
                # For entries that has lower value than the one before, set its value equal to the one before plus 1
                if np.any(np.diff(self.chi_of_z(z))<=0) :
                    entries = np.where(np.diff(self.chi_of_z(z))<=0)[0]
                    chi_of_z = self.chi_of_z(z)
                    for i in entries :
                        chi_of_z[i] = chi_of_z[i-1] + 0.01*(chi_of_z[i+1]-chi_of_z[i-1])
                ...
                # if np.any(self.chi_of_z(z)<=0) :
                #     block["distances", "pos_expf"] = False # Not exactly not positive-definite expansion function, but not positive-definite comoving distance
                #     block["shear_cl", 'nbin_a'] = 5
                #     block["shear_cl", 'nbin_b'] = 5
                #     ell = np.logspace(np.log10(1.0),np.log10(1.0e4),50)
                #     block["shear_cl", "ell"] = ell
                #     for i in range(5) :
                #         for j in range(i+1) :
                #             block["shear_cl", f'bin_{i+1}_{j+1}'] = np.zeros(50)
                #     print("!!!!!!!!!!!!!!!!COMOVING DISTANCE IS NOT POSITIVE DEFINITE!!!!!!!!!!!!!!!!")
                #     return 0

                # np.savetxt("chi2.txt", self.chi_of_z(z))
                # print("WWWWWWWWWWWWWWWWWWWWWWWWWW chi[0] = ", self.chi_of_z(z)[0])
                ...
                #MODIFICATION
                ###
                # print("RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR GOT YOU!")
                ###
            else:
    ...
    def load_power(self, block):
        ...
        for power_options in self.req_power_options:
            ...
            # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW type(power) = ", type(power))
            # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW power.section_name = ", power.section_name)
            if self.verbose:
    ...
    def compute_spectrum(self, block, spectrum):
        ...
        # MODIFICATION
        ###
        # print("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE spectrum.section_name : ", spectrum.section_name)
        ###
        ...
        for i in range(na):
            ...
            for j in range(jmax):
                ...
                # print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW ell's section name : ", sep_name)
                block[spectrum.section_name, f'bin_{i+1}_{j+1}'] = c_ell
                # print(f"SAVING C_ELL OF BIN_{i+1}_{j+1} into section {spectrum.section_name}") #MODDD
                # np.savetxt(f"cell_mycamb_nl_k500_zmin05_bin{i+1}_{j+1}",c_ell) #MODDDD
...
def setup(options):
    # lin = options[option_section, "lin"] # MODDD
