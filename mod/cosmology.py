...
class Cosmology:
    ...
    def __init__ (self, c1, c2, c3, a, \
        ...
        self.omega_l  = 1.0-self.omega_r-self.omega_m
    ...
    def growth_index (self):
        e_init = self.expansion_function (self.a)[0]
        v = [ \
             3.0+2.0*self.a[0]* \
             self.d_expansion_function_d_a(self.a)[0]/e_init, \
             1.0-self.omega_m/(self.a[0]*(e_init*self.a[0])**2) \
            ]
    ...
    #     cheb_poly = np.polynomial.chebyshev.Chebyshev([self.c1,self.c2,self.c3], domain=[a0,aEnd])
    #     # expf = self.hubble*cheb_poly(np.linspace(a0,aEnd,self.n))
    #     expf = self.hubble*cheb_poly(a)**2
    #     return expf
    # self.hubble*np.polynomial.Chebyshev([self.c1,self.c2,self.c3], domain:=[0,a[-1]])
    ...
    #     # d_expf = self.hubble*cheb_deriv(np.linspace(a0,aEnd,self.n))
    #     d_expf = self.hubble*cheb_deriv(a)**2
    #     return d_expf
    ...
    #     t = [self.a[0], a]
    #     delta_initial = \
    #         [self.a[0], self.n_growth*self.a[0]**(self.n_growth-1.0)]
    ...
    def growth_factor (self, a, n = 0):
        ...
        # print("ASDFASDFASDFASDFASDF n_growth ",self.n_growth)
        # print(f"DEBUGGGGGGGGGGGGGGGGGGG YOU GOT TO HERE! D+({a},n={n})", delta[1,n])
        return delta[1,n]
