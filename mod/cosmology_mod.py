...
class Cosmology:
    ...
    def expansion_function (self, a):
        ...
        # print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX ", np.any(self.cheb(self.c,2*x-1)==0))
        # expf = expf/expf[-1]
    ...
    #     x = (a-self.amin)/(self.amax-self.amin) 
    #     A = (1+x*self.da)
    #     prefactor = -self.da/self.hubble/(self.amax-self.amin)/A**2/self.cheb(self.c,2*x-1)
    #     term1 = 2*self.da/A
    #     term2 = 2*self.cheb_derivative(self.c,2*x-1)/self.cheb(self.c,2*x-1)
    #     d_expf = prefactor*(term1 + term2)/self.norm
    #     return d_expf
    
    def d_expansion_function_d_a (self,a,c=None):
        if c is None :
            c = self.c
        x = (a-self.amin)/(self.amax-self.amin)
        A = (1+x*self.da)
        prefactor = -self.da/self.hubble/(self.a[-1]-self.a[0])/A**2/self.cheb(c,2*x-1)
        term1 = 2*self.da/A
        term2 = 2*self.cheb_derivative(c,2*x-1)/self.cheb(c,2*x-1)
        d_expf = prefactor*(term1 + term2)/self.norm
        return d_expf
...
##########################################################################################################################3
    ...
    # # Using Numpy's implementation of chebyshev polynomial
    # def expansion_function (self, a):
    ...
    #     cheb_poly = np.polynomial.chebyshev.Chebyshev(self.c, domain=[0,1])
    #     da = (1-self.amin)/self.amin
    #     x = (a-self.amin)/(self.amax-self.amin)
